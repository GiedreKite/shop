    ...
    listEvents() {
        ...
        for (const rowDOM of rowsDOM) {
            ...
            buttonsDOM[0].addEventListener('click', () => {
                const idToDecrement = rowDOM.id;
                const localStorageData = localStorage.getItem('itemList');
                let list = JSON.parse(localStorageData)
                    .map(item => item.id === idToDecrement ? { ...item, amount: (item.amount>=1 ? item.amount - 1 : 0)} : item);

                localStorage.setItem('itemList', JSON.stringify(list));

                amountDOM.textContent = list.filter(item => item.id === idToDecrement)[0].amount;
